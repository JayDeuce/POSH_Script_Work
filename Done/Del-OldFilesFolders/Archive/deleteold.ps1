               ...
               switch ($Entry.Type) {
                    ...
                    'year' {
                         ...
                         switch ($Entry.Day) {
                              ...
                              Default {
                                   $Check.YearSuccess = $false
                              }
                         }
                    }
                    'date' {
                         # Verify if the date is in the correct format
                         switch ($Entry.Day) { {try {
                                        [DateTime]"$($Entry.Day)"
                                   } catch{
                                   }} {
                    ...
                    'daterange' {
                         ...
                         switch ($Entry.Day) { {try {
                              ...
                              Default {
                                   $Check.DateRangeSuccess = $false
                              }
                         }
                    }

                    'limityear' {
                         switch ($Entry.Day) { {(1000..2100) -contains $_} {
                                   $HashOutput.LimitYear = $Entry.Day
                              }
                              Default {
                                   $Check.LimitYearSuccess = $false
                              }
                         }
                    }
                    Default {
     ...
     if ($DayOfMonth -ne $null) {
          for ($MonthCnt = 0; $MonthCnt -lt $DayOfMonth.Count; $MonthCnt++) {
               ...
               for ($j = 0; $j -lt $MonthRepeat[$MonthCnt]; $j++) {
                    ...
                    if ($CheckDate -le $CurrentDate -and $(if ($DayOfMonth[$MonthCnt] -ne 0) {
                              $CheckDate.Day -eq $DayOfMonth[$MonthCnt]
                         } else {
                              $true
                         })) {
                    $CheckDate
...
if ($DayOfQuarter -ne $null) {
     ...
     } while ($(if ($QuarterRepeat -eq -1) {
               $LimitYear -le $(if ($CheckDate) {
                    $CheckDate.Year
               } else {
                    9999
               })
     ...
     for ($j=0;$j -lt $Count;$j++) {
          ...
          if (-not $?) {
               ...
               if (-not $ListOnly) {
                    ...
                    if ($VerboseLog) {
                         switch ($true) { {$CreateTime} {
                                   "`tDELETED FILE`t$($FileList[$j].CreationTime.ToString('yyyy-MM-dd hh:mm:ss'))`t$($FileList[$j].Length.ToString().PadLeft(15))`t$tempfile" >> $LogFile
                              } {$LastAccessTime} {
                                   "`tDELETED FILE`t$($FileList[$j].LastAccessTime.ToString('yyyy-MM-dd hh:mm:ss'))`t$($FileList[$j].Length.ToString().PadLeft(15))`t$tempfile" >> $LogFile
                              }
                              Default {
                                   "`tDELETED FILE`t$($FileList[$j].LastWriteTime.ToString('yyyy-MM-dd hh:mm:ss'))`t$($FileList[$j].Length.ToString().PadLeft(15))`t$tempfile" >> $LogFile
                              }
                         }
                    }
               }
          ...
          if($ListOnly) {
               if ($VerboseLog) {
                    switch ($true) { {$CreateTime} {
                              "`tLISTONLY`t$($FileList[$j].CreationTime.ToString('yyyy-MM-dd hh:mm:ss'))`t$($FileList[$j].Length.ToString().PadLeft(15))`t$tempfile" >> $LogFile
                         } {$LastAccessTime} {
                              "`tLISTONLY`t$($FileList[$j].LastAccessTime.ToString('yyyy-MM-dd hh:mm:ss'))`t$($FileList[$j].Length.ToString().PadLeft(15))`t$tempfile" >> $LogFile
                         }
                         Default {
                              "`tLISTONLY`t$($FileList[$j].LastWriteTime.ToString('yyyy-MM-dd hh:mm:ss'))`t$($FileList[$j].Length.ToString().PadLeft(15))`t$tempfile" >> $LogFile
                         }
                    }
               } else {
     ...
     for ($j=0;$j -lt $Count;$j++) {
          ...
          if (-not $TempFolder) {
               ...
               if(-not $?) {
                    ...
                    if ($VerboseLog) {
                         switch ($true) { {$CreateTime} {
                                   "`tDELETED FOLDER`t$($FolderList[$j].CreationTime.ToString('yyyy-MM-dd hh:mm:ss'))`t`t`t$TempName" >> $LogFile
                              } {$LastAccessTime} {
                                   "`tDELETED FOLDER`t$($FolderList[$j].LastAccessTime.ToString('yyyy-MM-dd hh:mm:ss'))`t`t`t$TempName" >> $LogFile
                              }
                              Default {
                                   "`tDELETED FOLDER`t$($FolderList[$j].LastWriteTime.ToString('yyyy-MM-dd hh:mm:ss'))`t`t`t$TempName" >> $LogFile
                              }
                         }
                    }
...
if (-not $FolderPath) {
     Write-Warning 'Please specify the -FolderPath variable, this parameter is required. Use Get-Help .\deleteold.ps1 to display help.';exit
}
if (-not $FileAge) {
     Write-Warning 'Please specify the -FileAge variable, this parameter is required. Use Get-Help .\deleteold.ps1 to display help.';exit
}
if (-not $LogFile) {
     Write-Warning 'Please specify the -LogFile variable, this parameter is required. Use Get-Help .\deleteold.ps1 to display help.';exit
}
if ($Autolog) {
     F_Autolog
}
...
if ($EmailTo) {
	    ...
	    for ($j=0;$j -lt $EmailTo.Count;$j++) {
          $Switches+= "`r`n`t`t`t";$Switches+= $EmailTo[$j]
     }
}
if ($EmailFrom) {
...
if ($ListOnly) {
     $Switches+="`r`n`t`t-ListOnly"
}
if ($VerboseLog) {
     $Switches+="`r`n`t`t-VerboseLog"
}
if ($Autolog) {
     $Switches+="`r`n`t`t-AutoLog"
}
if ($Appendlog) {
     $Switches+="`r`n`t`t-AppendLog"
}
if ($CreateTime) {
     $Switches+="`r`n`t`t-CreateTime"
}
if ($LastAccessTime) {
     $Switches+="`r`n`t`t-LastAccessTime"
}
if ($CleanFolders) {
     $Switches+="`r`n`t`t-CleanFolders"
}
if ($NoFolder) {
     $Switches+="`r`n`t`t-NoFolder"
}
if ($RegExPath) {
     $Switches+="`r`n`t`t-RegExPath"
}
[long]$FilesSize = 0
[long]$FailedSize = 0
[int]$FilesNumber = 0
...
if ($IncludeFileExtension) {
     for ($j=0;$j -lt $IncludeFileExtension.Count;$j++) {
          ...
          if ($IncludeFileExtension[$j].Substring(0,1) -ne '.') {
               $IncludeFileExtension[$j] = ".$($IncludeFileExtension[$j])"
          }
...
if ($ExcludeFileExtension) {
     for ($j=0;$j -lt $ExcludeFileExtension.Count;$j++) {
          # If no dot is present the dot will be added to the front of the string
          if ($ExcludeFileExtension[$j].Substring(0,1) -ne '.') {
               $ExcludeFileExtension[$j] = ".$($ExcludeFileExtension[$j])"
          }
...
if ($CheckError -gt 0) {
     for ($j=0;$j -lt $CheckError;$j++) {
          ...
          if ($ExcludePath) {
               if ($(for ($k=0;$k -lt $ExcludePath.Count;$k++) {
                         $Error[$j].TargetObject -match $ExcludePath[$k].SubString(0,$ExcludePath[$k].Length-2)
                    }) -notcontains $true) {
